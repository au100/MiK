	...
	if (!isset($user_cache[$poster_id]))
	{
		...
		else
		{
			...
			$user_cache[$poster_id] = array(
				...
				'selected_mypage'	=> ($row['user_selected_mypage']) ? 'mypage.php?id='.$row['user_selected_mypage'] . '&sid=' . $user->data['session_id'] : false,
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow = array(
		...
		'POST_SUBJECT'		=> $row['post_subject'],
		'MESSAGE'		=> $message,
		'SIGNATURE'		=> ($row['enable_sig']) ? ($config['reimg_ignore_sig_img'] ? str_replace(reimg_properties(), '', $user_cache[$poster_id]['sig']) : $user_cache[$poster_id]['sig']) : '',		'EDITED_MESSAGE'	=> $l_edited_by,
		...
		// begin JTS Enterprises My page mod //
		'U_MYPAGE_SELECT'	=> $user_cache[$poster_id]['selected_mypage'],
		// end JTS Enterprises My page mod //
