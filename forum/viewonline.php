...
// Get user list
$sql = 'SELECT u.user_id, u.username, u.username_clean, u.user_type, u.user_colour, s.session_id, s.session_time, s.session_page, s.session_ip, s.session_browser, s.session_viewonline, s.session_forum_id, s.session_start
	FROM ' . USERS_TABLE . ' u, ' . SESSIONS_TABLE . ' s
...
while ($row = $db->sql_fetchrow($result))
{
	if ($row['user_id'] != ANONYMOUS && !isset($prev_id[$row['user_id']]))
	{
		...
		$username_full = ($row['user_type'] != USER_IGNORE) ? 
		      get_username_string('full', $row['user_id'], $row['username'], $row['user_colour']) : 
		      '<span' . $user_colour . '>' . $row['username'] . '</span>';
		...
		$username_full_mod = $username_full;
...
// add by au100
//                $sql = 'SELECT u.user_id, u.username, u.user_colour, s.
//	           FROM ' . USERS_TABLE . ' u, ' . SESSIONS_TABLE . ' s
//	WHERE u.user_id = s.session_user_id
//		AND s.session_time >= ' . (time() - ($config['load_online_time'] * 60)) .
//		((!$show_guests) ? ' AND s.session_user_id <> ' . ANONYMOUS : '') . '
...
//
		...
		if(isset($row['username']))
		  {
 		  $user_colour = ($row['user_colour']) ? ' style="color:#' . $row['user_colour'] . '" class="username-coloured"' : '';
		  $username_full_mod = '<em><span' . $user_colour . '>' . 'Гость - (' . $row['username'] . ')</span></em>';
	  	  }
		else
		  {
		  $username_full_mod = get_username_string('full', $row['user_id'], $user->lang['GUEST']);
		  }
...
//                break;
				...
				switch ($on_page[1])
				{
					case 'posting':
					  ...
					  switch ($posting_mode)
					    ...
					    case 'quote':
  					      // -------------------------------------
                                              // Added by au100 06.11.2011
					      // Added to view topic "Who is Online"  
					      // -------------------------------------
					        ...
					        // -------------------------------------
  					        $location = sprintf($user->lang['REPLYING_MESSAGE'], 
                                                                    $forum_data[$forum_id]['forum_name'], 
                                                                    $topic_data['topic_title']);
 					        $location_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 
                                                                         'f=' . $forum_id . '&t=' . $on_page[1]);
                                              }
                                              else
                                              {
  					        $location = sprintf($user->lang['REPLYING_MESSAGE'], '',
					...
					case 'viewtopic':
					  ...
					  // -------------------------------------
					    ...
					    // -------------------------------------
					    $location = sprintf($user->lang['READING_TOPIC'], 
                                                                $forum_data[$forum_id]['forum_name'], 
                                                                $topic_data['topic_title']);
   					    // -------------------------------------
					    //  Added to view topic "Who is Online"  
	...
	$template->assign_block_vars('user_row', array(
		...
		'USERNAME_FULL_MOD'     => $username_full_mod,
