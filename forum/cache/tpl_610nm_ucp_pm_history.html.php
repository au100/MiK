...
<tr>
	...
	<?php $_history_row_count = (isset($this->_tpldata['history_row'])) ? sizeof($this->_tpldata['history_row']) : 0;if ($_history_row_count) {for ($_history_row_i = 0; $_history_row_i < $_history_row_count; ++$_history_row_i){$_history_row_val = &$this->_tpldata['history_row'][$_history_row_i]; ?>
			...
			<td class="<?php if (!($_history_row_val['S_ROW_COUNT'] & 1)  ) {  ?>row1<?php } else { ?>row2<?php } ?>" width="100%"<?php if ($_history_row_val['S_CURRENT_MSG']) {  ?> style="background: white"<?php } ?>>
		...
		<tr>
			...
			<td class="gensmall<?php if (!($_history_row_val['S_ROW_COUNT'] & 1)  ) {  ?> row1<?php } else { ?> row2<?php } ?>"><div class="gensmall" style="float: <?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>;">&nbsp;<?php if ($_history_row_val['U_PROFILE']) {  ?><a href="<?php echo $_history_row_val['U_PROFILE']; ?>"><?php echo (isset($this->_rootref['PROFILE_IMG'])) ? $this->_rootref['PROFILE_IMG'] : ''; ?></a> <?php } if ($_history_row_val['U_EMAIL']) {  ?><a href="<?php echo $_history_row_val['U_EMAIL']; ?>"><?php echo (isset($this->_rootref['EMAIL_IMG'])) ? $this->_rootref['EMAIL_IMG'] : ''; ?></a> <?php } ?>&nbsp;</div> 
