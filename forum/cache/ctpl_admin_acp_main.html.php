...
<?php } else { ?>
	...
	<?php if ($this->_rootref['S_ACTION_OPTIONS']) {  ?>
		...
		<fieldset>
			...
			<form id="action_reputation_form" method="post" action="<?php echo (isset($this->_rootref['U_ACTION'])) ? $this->_rootref['U_ACTION'] : ''; ?>">
				<dl>
					<dt><label for="action_reputation"><?php echo ((isset($this->_rootref['L_RP_RESET'])) ? $this->_rootref['L_RP_RESET'] : ((isset($user->lang['RP_RESET'])) ? $user->lang['RP_RESET'] : '{ RP_RESET }')); ?></label><br /><span><?php echo ((isset($this->_rootref['L_RP_RESET_EXPLAIN'])) ? $this->_rootref['L_RP_RESET_EXPLAIN'] : ((isset($user->lang['RP_RESET_EXPLAIN'])) ? $user->lang['RP_RESET_EXPLAIN'] : '{ RP_RESET_EXPLAIN }')); ?></span></dt>
					<dd><input type="hidden" name="action" value="reputation" /><input class="button2" type="submit" id="action_reputation" name="action_reputation" value="<?php echo ((isset($this->_rootref['L_RUN'])) ? $this->_rootref['L_RUN'] : ((isset($user->lang['RUN'])) ? $user->lang['RUN'] : '{ RUN }')); ?>" /></dd>
				</dl>
			</form>
			<!-- end of added   -->
